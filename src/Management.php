...
class Management implements LoggerAwareInterface
{
    ...
    public function processMessage(MessageInterface $message, MimePart $payload)
    {
        ...
        if (!$payload->isEncrypted() && $message->getSender()->getEncryptionAlgorithm()) {
            throw new \RuntimeException(sprintf('Incoming message from AS2 partner `%s` are defined to be encrypted', $message->getSender()->getAs2Id()));
        }
                    ...
                    if ($bodyPayload->hasHeader('disposition')) {
                        ...
                        if ($mdnStatus === 'processed') {
                            ...
                            if ($receivedMic &&
                                ...
                                throw new \RuntimeException(sprintf('The Message Integrity Code (MIC) does not match the sent AS2 message (required: %s, returned: %s)', $message->getMic(), $receivedMic));
                            }
